...
import { BehaviorSubject, EMPTY, Observable, ReplaySubject, catchError, distinctUntilChanged, filter, interval, map, startWith, switchMap } from 'rxjs';
import { ComicsDatabase, ComicsSearch, Series, DownloadLink, DownloadRequest, ComicSearchDetailsLinks, DownloadRequestList, ScrapperResponse, DownloadIssueRequest, DownloadIssue, DeleteReadOptions } from 'interfaces';
...
export class ComicsService {
  ...
  constructor() {
    this.activePageService.activePage$.pipe(
  ...
  allComics(): Observable<ComicsDatabase[]> {
    return this.httpService.request<ComicsDatabase[]>(
      'GET',
      `/comics`,
    );
  }
  ...
  allSeries(): Observable<Series[]> {
    return this.httpService.request<Series[]>(
      'GET',
      `/series`,
    );
  }
  ...
  getComicSearchDetailsLinks(idGc: string): Observable<ComicSearchDetailsLinks> {
    return this.httpService.request<ComicSearchDetailsLinks>(
      'GET',
      `/searchs/${idGc}/details`,
    );
  }
  ...
  downloadWithNotification(comic: ComicsSearch, comicSearchDetailsLinks: ComicSearchDetailsLinks): void {
    let observable$ = undefined;
    if (comicSearchDetailsLinks.downloadIssues.length === 1) {
      observable$ = this.modalService.open<DownloadLink, { options: DownloadLink[] | undefined; comicTitle: string }>(ModalDownloadComponent, { options: comicSearchDetailsLinks.downloadIssues[0].links, comicTitle: comicSearchDetailsLinks.title }).pipe(
        ...
        switchMap(response => {
          ...
          const downloadRequest: DownloadRequest = {
            comicSearchDetails: comicSearchDetailsLinks ?? { ...comic, description: '', tags: [], mainTag: null },
    ...
    if (comicSearchDetailsLinks.downloadIssues.length > 1) {
      observable$ = this.modalService.open<DownloadIssueRequest[], { options: DownloadIssue[] | undefined; comicTitle: string }>(ModalDownloadListComponent, { options: comicSearchDetailsLinks.downloadIssues, comicTitle: comicSearchDetailsLinks.title }).pipe(
        filter(response => response !== undefined),
        switchMap(response => {
          this.notifierService.appendProcessingNotification({
            id: 0,
          ...
          const downloadRequestList: DownloadRequestList = {
            comicSearchDetails: comicSearchDetailsLinks ?? { ...comic, description: '', tags: [], mainTag: null },
            downloadRequests: response.filter(downloadIssueRequest => downloadIssueRequest.link !== undefined)
          }
  ...
  downloads(): Observable<ComicsSearch[]> {
    return this.httpService.request<ComicsSearch[]>(
      'GET',
      `/downloads`,
    );
  }
