...
export class DownloadService {
  ...
  getComicPage(id: string, page: number): Observable<Blob> {
    return this.authService.handleApiCallError(() => {
      return this.httpClient.get<Blob>(`${this.configURLService.baseURL}/${this.configURLService.apiVersion}/comics/${id}/pages/${page}`, { responseType: 'blob' as 'json' });
    });
  }

  downloadComic(comic: ComicsDatabase): void {
    this.authService.handleApiCallError(() => {
      return this.httpClient.get(`${this.configURLService.baseURL}/${this.configURLService.apiVersion}/comics/${comic.id}/download`, { observe: 'response', responseType: 'blob' })
    }).pipe(untilDestroyed(this)).subscribe(response => {
      const contentDisposition = response.headers.get('content-Disposition');
      const filenameFromPath = this.getFileNameFromPath(comic.path);
      const filename = contentDisposition ? this.getFilenameFromContentDisposition(contentDisposition) : 'comic.cbr';
      if (response.body) {
  ...
  downloadComicProgress(comic: ComicsDatabase): void {
    ...
    this.authService
      .handleApiCallError(() =>
        this.httpClient.get(`${this.configURLService.baseURL}/${this.configURLService.apiVersion}/comics/${comic.id}/download`, {
          observe: 'events',
          responseType: 'blob',
          reportProgress: true
        })
        ...
        next: (event) => {
          if (event.type === HttpEventType.DownloadProgress) {
            ...
            if (response.body) {
              ...
              const comicLocalDownload = this.comicLocalDownload$.value.filter(item => item.comic.id !== comic.id);
    ...
    this.comicLocalDownload$.next([...this.comicLocalDownload$.value, { comic, progress$: progress$.asObservable() }]);
  ...
  downloadComicList(comics: ComicsDatabase[]): void {
    ...
    this.authService
      .handleApiCallError(() =>
        this.httpClient.post(`${this.configURLService.baseURL}/${this.configURLService.apiVersion}/comicList/download`, comicIDs, {
          observe: 'events',
          responseType: 'blob',
          reportProgress: true
        })
