...
import { ComicsService, NotifierService, READER_SETTINGS_BOOLEANS, ReaderSettingsBooleanMap, ReaderSettingsBooleans, ReaderSettingsService } from 'services';
...
@Component({
  ...
  imports: [CrossModalComponent, CommonModule, ReactiveFormsModule, BooleanSliderFormComponent, HideRolesDirective],
  templateUrl: './modal-reader-settings.component.html',
})

@UntilDestroy()
export class ModalReaderSettings implements ModalComponent<ReaderSettings, { comic: ComicsDatabase, page: number }>, OnInit {
  ...
  constructor() {
    this.form = this.formBuilder.group({
      readerSettingsService: this.formBuilder.group(
        Object.fromEntries(
          READER_SETTINGS_BOOLEANS.map(k => [k, new FormControl(true)])
        ) as { [K in ReaderSettingsBooleans]: FormControl<boolean> }
      ),
  ...
  setCover() {
    if (this.data) {
      this.comicsService.setCover(this.data.comic.id, this.data.page).pipe(
        untilDestroyed(this)
      ).subscribe(() => {
